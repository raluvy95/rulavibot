...
module.exports = {
  ...
  example: "v!help botinfo",
    ...
    execute: (message, args) => {
        let bruh = args[0]
        if(bruh == "admin") {
          if(message.author.id != "390540063609454593") return message.reply("No command found")
            let admin = message.client.commands.filter(m => m.ownerOnly)
            let e = new discord.RichEmbed()
            .setTitle("Admin commands")
            .setDescription(admin.map(m => `\`${m.name}\``).join(", "))
            .setFooter("Only Owner of this bot could execute it")
            ...
            .addField("Example", !cmmd.example ? "No example" : cmmd.example)
          return message.channel.send(e1)
        }
        const modules = fs.readdirSync(`./modules/`).filter(file => !file.startsWith("_"))
        const e = new discord.RichEmbed()
        .setTitle(`Avaliable commands (${message.client.commands.size} commands)`)
        .setDescription("Ping a bot developer for report a bug.")
