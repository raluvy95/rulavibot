...
module.exports = {
  name: "weekly",
  module: "gdps",
  description: "Show the current weekly level",
  cooldown: 10,
  execute: async (message, args, gd) => {
    let a = await gd.api.levels.getWeekly()
    if(!a) return message.channel.send("There's no current weekly level")
    let diff = {
    ...
    .setAuthor("Weekly Level!", "https://images-ext-2.discordapp.net/external/-V-AYSPiOoLwoowRlUWtlDAFJx7N4xyVqpZuKuFMitM/https/i.imgur.com/kcsP5SN.png")
    .setTitle(`${a.name}`)
    ...
    .addField("Stats", `**Difficulity:** ${diff[a.diff]} (${a.stars})\n**Downloads:** ${a.downloads}\n**Likes:** ${a.likes}\n**Length:** ${a.length}\n**Song ID:** ${a.songID}\n**Original Level ID:** ${a.original}`)
